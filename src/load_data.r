housing_df = read.csv('../data/AmesHousing.tsv',sep='\t')
...
rownames(housing_df) <- housing_df$Order
housing_df$PID <- NULL
housing_df$Order <- NULL
...
numeric_sc_df <- data.frame(scale(Filter(is.numeric, housing_df)))
numeric_log_sc_df <- data.frame(scale(log(Filter(is.numeric, housing_df)+1)))
#numeric_log_sc_df$SalePrice <- NULL

factors_df <- Filter(is.factor, housing_df)